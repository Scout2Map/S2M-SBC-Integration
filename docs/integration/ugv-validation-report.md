...
이 문서는 현재 호스트 테스트로 확인할 수 없는 MCU/차체 항목과 시스템 통합 시험을
정리한다. MCU 구현은 S2M-FW-DrivingControl이 소유하고, 이 저장소는 SLAM, marker,
자동 복귀와 ROS 통합 시험 결과를 추적한다.
...
| 입력 | 상태 | 필요한 값 |
|---|---|---|
| STM32 보드 | 미확정 | Hardware에 F103C8T6 자료는 있으나 실제 MCU/보드 리비전 확인 필요 |
| encoder | 미확정 | JGB37-520B 자료는 있으나 실제 장착 여부, PPR/CPR, 감속비, A/B 극성 확인 필요 |
...
- 실제 USB ISR/ring buffer, PWM 0 전파 시간, encoder scale, PID 안정성은 호스트
  테스트로 증명할 수 없다.
- `S2M-Hardware`의 xacro/슬립 world 존재는 확인했지만 Gazebo에서 직접 실행한
  결과는 이 보고서에 아직 없다.
- 자동 복귀는 [시뮬레이션 계획](return-home-simulation-plan.md)만 있고 실행 결과는 없다.
- 실차 SLAM 안정성은 7월 31일 회의에서 하드웨어 주행 결과가 선행 조건으로
  명시됐으므로 UGV 접근 전에는 완료 처리하지 않는다.
