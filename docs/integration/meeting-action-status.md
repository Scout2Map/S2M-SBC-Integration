...
기준 원문은 로컬 보관된 `260731 Scout2Map 회의록.pdf`와
`2026_08_05_회의록.pdf`다. 페이지 번호는 PDF 기준이다.
...
이 문서는 기술 저장소 작업을 추적한다. 7월 31일 p.2의 인당 8만원 비용 각출은
회의록만으로 완료 여부를 확인할 수 없으며 저장소 범위 밖이다. 공유 논문 구조
정리는 팀 공통 미확인 항목이지만 8월 5일 p.2에서 이번 주기 순연으로 결정됐다.
