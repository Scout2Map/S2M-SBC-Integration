...
- 모델 파일은 저장소에 넣지 않으며 실행 시 SHA-256을 `/vision/info`에 발행
...
먼저 모델과 클래스 파일을 준비한다. 기본 카메라 설정은 Pi 5 부하 측정을 위해
640x480 10 FPS이며, AU142의 FHD 최대 해상도는 성능 검증 후 올린다.

```bash
ros2 launch scout_vision vision.launch.py \
  model_path:=/opt/scout2map/models/hazard_yolov8.onnx \
  labels_path:=/opt/scout2map/models/hazard_labels.txt
```
...
ros2 launch s2m_bringup s2m_slam_real.launch.py \
  ...
  vision_model:=/opt/scout2map/models/hazard_yolov8.onnx \
  vision_labels:=/opt/scout2map/models/hazard_labels.txt
```
