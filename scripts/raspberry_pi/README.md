...
Ubuntu 24.04, `arm64`, 대상 SSD가 루트 파일시스템인지 확인하고 업데이트합니다.
...
스크립트는 `~/scout2map_ws`를 만들고 로컬 패키지와 필요한 ROS 소스 패키지를
...
센서 Pico와 주행 STM32는 각각 독립된 USB CDC 링크를 사용합니다. 실제 장치의
VID/PID/serial을 확인해 udev 규칙으로
`/dev/scout2map_pico`, `/dev/scout2map_drive` 같은 안정된 이름을 만듭니다.

규칙 작성과 설치 방법은 [`udev/README.md`](udev/README.md)를 참고합니다.
